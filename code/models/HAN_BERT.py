...
class HAN_BERT(nn.Module):
    ...
    def __init__(self, vocab_size=5000, emb_dim=300, hid_dim=128, maps=None, details={}):
        ...
        self.fc_ar1 = nn.Linear(2 * sentence_rnn_size, sentence_rnn_size)
        self.fc_ch1 = nn.Linear(2 * sentence_rnn_size, sentence_rnn_size)
        self.fc_pt1 = nn.Linear(2 * sentence_rnn_size, sentence_rnn_size)
    ...
    def forward(self, data):
        ...
        documents = data["fact"]["input_ids"].cuda()
        sentences_per_document = data["fact"]["doc_len"].cuda()
        words_per_sentence = data["fact"]["sent_len"].cuda()
        ...
        # Classify
        # out_ar = self.fc_article(self.dropout(document_embeddings))  # (n_documents, n_classes)
        # out_ch = self.fc_charge(self.dropout(document_embeddings))  # (n_documents, n_classes)
        # out_pt = self.fc_penalty(self.dropout(document_embeddings))  # (n_documents, n_classes)

        out_ar = self.fc_ar2(nn.ReLU()(self.fc_ar1(document_embeddings)))
        out_ch = self.fc_ch2(nn.ReLU()(self.fc_ch1(document_embeddings)))
        out_pt = self.fc_pt2(nn.ReLU()(self.fc_pt1(document_embeddings)))
...
class WordAttention(nn.Module):
    ...
    def __init__(self, vocab_size, emb_size, word_rnn_size, word_rnn_layers, word_att_size, dropout):
        ...
        hid_dim = 768
        self.word_rnn = nn.GRU(hid_dim, word_rnn_size, num_layers=word_rnn_layers, bidirectional=True,
                               dropout=dropout, batch_first=True)
        self.bert = AutoModel.from_pretrained("code/ptm/bert-base-chinese")
    ...
    def forward(self, sentences, words_per_sentence):
        ...
        # sentences : (112, 100)
        # sentences_emb : (112, 100, 300)
        sentences = self.bert(sentences).last_hidden_state
        sentences = self.dropout(sentences)
        ...
        packed_words, _ = self.word_rnn(packed_words) # (packed_words.data 5959, 600)

        # Find attention vectors by applying the attention linear layer on the output of the RNN
        att_w = self.word_attention(packed_words.data)  # (5959, 600)
        ...
        att_w = torch.exp(att_w - max_value)  # (n_words) (5959)
        ...
        att_w, _ = pad_packed_sequence(PackedSequence(data=att_w,
                                       ...
                                       batch_first=True)  # (n_sentences, max(words_per_sentence)) (112, 100)
        ...
        return sentences, word_alphas # (112, 768), (112, 100)
